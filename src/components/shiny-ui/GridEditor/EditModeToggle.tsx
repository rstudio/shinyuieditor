...
export function EditModeToggle({ selected, onSelect }: { selected: EditMode; onSelect: (selection: EditMode) => void; }) {
    return <Container>
        ...
        <ButtonsHolder className={selected === "UI" ? "left-selected" : "right-selected"}>
            <button value="UI" className="left" onClick={() => onSelect("UI")}>UI</button>
            <button value="Layout" className="right" onClick={() => onSelect("Layout")}>Layout</button>
        </ButtonsHolder>
    </Container>;
}
