...
export function flatten_array(node: R_AST): Primative_Array {
  ...
  if (call.val !== "c") {
    throw new Parsing_Error({ message: "Tried to flatten non array as array" });
  }
...
function get_node_is_list(node: R_AST): boolean {
  return node[0].val === "list";
}
