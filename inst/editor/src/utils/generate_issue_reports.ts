 ...
 */
function generate_serialized_state_for_error(app_state: RootState) {
 ...
 */
function generate_gh_issue_url(info: {
  title: string;
  body: string;
  labels?: string[];
}) {
  const title_text = encodeURIComponent(info.title);
  const body_text = encodeURIComponent(info.body);
  const labels_text = encodeURIComponent(["autogenerated"].join(","));

  return `https://github.com/rstudio/shinyuieditor/issues/new?labels=${labels_text}&title=${title_text}&body=${body_text}`;
}
