...
describe("Can convert from basic script and ast to ui aware app info", () => {
  // It's super important the script and ast match eachother here.
  const raw_info: Raw_App_Info = {
...
`,
  ...
  test("Libraries", () => {
    expect(full_info.libraries).toStrictEqual(["shiny", "gridlayout"]);
  });

  test("Can convert back to code", () => {
    const back_to_script = generate_full_app_script(full_info);
    debugger;
    expect(back_to_script).toEqual(raw_info.script);
  });
});
